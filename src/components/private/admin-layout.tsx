...
import { LayoutDashboard, Calendar, Cog, Palette, LogOut, User, UserCheck } from "lucide-react"
...
export function AdminLayout({ title, userEmail, onLogout, children }: AdminLayoutProps) {
  ...
  const items = [
    ...
    },
    {
      ...
      to: "/attendance-control",
      icon: <UserCheck className="h-5 w-5" />,
      active: location.pathname === "/attendance-control",
    },
  ...
  return (
    <div className="relative min-h-screen w-full max-w-[100vw] overflow-hidden overflow-x-clip bg-gradient-to-b from-ipimWhiteSnow via-background to-background dark:from-[#0b0b0d] dark:via-[#0b0b0d] dark:to-black">
      ...
      <div className="relative z-10 flex">
        ...
        <main className="flex-1 min-w-0 overflow-x-clip">
              ...
              <div className="flex items-center gap-2">
                ...
                {onLogout && (
                  <Button
                    variant="outline"
                    onClick={onLogout}
                    className="border-border/70 bg-background/70 text-foreground hover:border-rose-400/40 hover:bg-rose-500/10 hover:text-rose-500 dark:border-white/15 dark:bg-white/[0.03] dark:text-white dark:hover:text-rose-400"
                  >
          ...
          <div
            className={cn(
              "phone:py-6 laptop:py-8 overflow-x-clip",
              collapsed ? "phone:px-4 tablet:px-8" : "phone:px-3 tablet:px-10",
            )}
          >
            <div className="mx-auto w-full max-w-[1200px] overflow-x-clip">
